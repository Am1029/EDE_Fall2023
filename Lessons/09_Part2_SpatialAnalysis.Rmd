...
ggplot() + 
  ...
  geom_sf(data = neuse_sf, fill= "purple")

mapview(huc8_sf, col.region = "orange", layer.name= "HUC 8") +
mapview(neuse_sf, col.region = "purple", layer.name= "Neuse")

# Extra fancy
mapview(huc8_sf, col.region = "orange") |
mapview(neuse_sf, col.region = "purple")
```
...
huc2_sf <-huc8_sf %>% group_by(DWQ_Basin) %>% summarize(ACRES = sum(ACRES),
...
tri_sf <- counties_sf_utm %>% filter(NAME %in% c("Chatham", "Durham", "Orange","Wake"))
