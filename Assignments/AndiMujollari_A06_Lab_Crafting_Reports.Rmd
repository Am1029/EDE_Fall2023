...
```{r project.setup, include = TRUE, message=FALSE}
#To include the code chunk in the document i used "include=TRUE" & to not show any messages or output to the document i used the "message=False" command. 
...
#First i create a variable that has the names of my columns 
my_col.names <- c('County','$\\mu g/m^3$')

#Here i create the PM2.5 table, name the columns and put the title
knitr::kable(summary_PM25, 
    col.names = my_col.names, 
    caption = 'Mean Particulates (2.5mm)')

#Here i create the Ozone table, name the columns and add the title
...
```{r echo=FALSE, fig.cap="Monthly Ozone", warning=FALSE}
# Filter the data for records collected in 2018
ozone_2018 <- epa_data %>% 
  filter(year(Date) == 2018)

#Here i create the boxplot through the ggplot command, and make sure the units are correct
ggplot(ozone_2018, aes(x = factor(Month), y=Ozone)) +
  geom_boxplot()+
  labs(title = "Recorded Ozone by Month (2018)",y = expression(
    paste("Concentration is (" ,mu, "g/" ,m^{3},")"))) +
  scale_x_discrete(name="Month",expand = c(0, 0), labels = month.abb, drop = FALSE)

```

```{r fig.cap="Monthly Ozone, 2019", warning=FALSE, echo=FALSE}
# Filter the data for records collected in 2019
ozone_2019 <- epa_data %>% 
  filter(year(Date) == 2019)

#Here i create the boxplot through the ggplot command, and make sure the units are correct
ggplot(ozone_2019, aes(x = factor(Month), y=Ozone)) +
  geom_boxplot()+
  labs(title = "Recorded Ozone by Month (2019)",y = expression(
    paste("Concentration is (" ,mu, "g/" ,m^{3},")"))) +
  scale_x_discrete(name="Month",expand = c(0, 0), labels = month.abb, drop = FALSE)
...
https://github.com/Am1029/EDE_Fall2023 
